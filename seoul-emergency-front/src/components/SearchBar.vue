<template>
  <div class="search">
    <b-input-group>
      ...
      <b-form-select
        v-model="$store.state.category2"
        :options="$store.state.options2"
        @change="onChange2($store.state.category2)"
      ></b-form-select>
  ...
  methods: {
    ...
    async fillMapCenterAddress() {
      return await this.$store.dispatch(
        'getReverseGeocoding',
        this.$store.state.currentLon + ',' + this.$store.state.currentLat
      );
    },
  },
};
